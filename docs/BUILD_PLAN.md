...
2. **Dashboard's "Total Active Clients" metric — clients have no `is_active` column.** The DDL never gives `clients` a status flag (unlike `users`, `portals`, `tender_names`). "Active" here presumably just means "exists" (not soft-deleted, since client deletion is a hard cascade delete per the cascade table). Confirm the metric is simply `count(*)` on `clients`, not filtered by some status that doesn't exist in the schema. Blocks Phase 7 (and the "Total Active Clients" badge in Phase 2's Clients page, which has the same wording).

3. **Frontend pagination param naming mismatch.** `frontend/CLAUDE.md`'s own example hook signature uses `pageSize` (camelCase) and passes it straight into axios `params`, which would serialize literally as `?pageSize=25` — but `API_CONTRACT.md` and the backend consistently expect `page_size` (snake_case). Every `api/*.ts` list function needs to map `pageSize` → `page_size` at the axios-params boundary, or the pattern needs to just use `page_size` in the params type directly. Flagging so it isn't silently copy-pasted wrong across all 6 list endpoints. Doesn't block any phase by itself — just needs a decided convention before Phase 2's `clients.ts` sets the pattern everyone else copies.
...
5. **No redirect from `/` to `/dashboard` after successful login.** `frontend/CLAUDE.md`'s router example keeps `/` as the login page unconditionally and doesn't show `/dashboard` as a post-login redirect target, but the PRD calls `/dashboard` "Landing page after login." Confirm: does `<LoginPage>` itself redirect on `isAuthenticated`, or does `<ProtectedRoute>` handle `/` specially? Small decision, affects Phase 1 (login page behavior) and Phase 7 (dashboard route registration).
...
7. **JWT claim contents unspecified.** Neither `ARCHITECTURE.md` nor `backend/CLAUDE.md` states what goes in the JWT payload beyond implying a user identifier (`get_current_user` "loads the user from DB" from the token, so at minimum a `sub` claim is needed). Since the user is always reloaded from the DB on every request anyway (no claim-trusting), this is a low-stakes implementation detail — flagging only so Phase 1 doesn't need a second round-trip decision. Proposed default: `sub` = user id (string UUID), `exp`, `type` (`access`/`refresh`) to distinguish token kinds on the `/refresh` endpoint. Will proceed with this default unless told otherwise.

None of these are blocking for Phase 0. Items 1, 4, and 6 are worth resolving explicitly before their respective phases start; items 2, 3, 5, 7 are small enough to decide inline when we get there, called out here so they don't get silently guessed differently by two different sessions later.
