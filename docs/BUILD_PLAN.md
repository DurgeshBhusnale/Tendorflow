...
4. ~~**Credentials click-to-reveal mechanism is underspecified.**~~ **RESOLVED (Phase 4):** option (b) — a per-row `GET /api/credentials/:id?reveal=true`. List-level `reveal` was **removed** from the contract entirely, so no response can ever unmask more than one password at a time. `API_CONTRACT.md` §5 updated accordingly; a regression test asserts that forcing `?reveal=true` on the list still returns only masks.
...
6. ~~**Any-employee password reveal is a real exposure, not just a wording nit.**~~ **RESOLVED (Phase 4):** explicitly confirmed as intended — any authenticated user of either role may reveal any stored portal password. Ownership continues to gate *writes* only. Documented at the top of `API_CONTRACT.md` §5 and covered by a test named for the decision, so it reads as deliberate rather than accidental. Encryption at rest remains a hardening item (`ARCHITECTURE.md` §7 item 1).
