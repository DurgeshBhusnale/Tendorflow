...
- **Vercel Python cold start slow** → expected. First request after inactivity takes ~2–3s. Subsequent requests are fast. If it's painful, move backend to Railway/Fly.
- **Every request that touches the database is slow (seconds, not milliseconds)** → check that the function region and the Supabase region still match. `backend/vercel.json` pins `"regions": ["sin1"]` against a Supabase project in `ap-southeast-1`. Vercel's default is `iad1` (Washington DC); split across continents this cost ~3.5s per request. Confirm the region actually in use from the response header — `X-Vercel-Id: bom1::sin1::…` names the execution region as the second field.
- **`asyncpg` connection issues on Vercel** → make sure `DATABASE_URL` uses the Supabase **Transaction pooler** endpoint (port 6543). See `backend/app/database.py`.
- **`InterfaceError`, or "attached to a different loop", after a period of traffic** → the connection pool added in `app/database.py` assumes the runtime reuses its asyncio event loop between invocations. If it doesn't, pooled connections outlive their loop and break. Revert `app/database.py` to `poolclass=NullPool`; `docs/ARCHITECTURE.md` §2 has the full reasoning.
